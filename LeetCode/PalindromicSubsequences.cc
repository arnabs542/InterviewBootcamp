...
static const long modulus = exp(10, 9) + 7;
set<string> palindromes;

int palindromicSubsequencesRec(const string& S, int index, string& str)
{
  if (index == S.length())
    return 0;
  ...
  c += palindromicSubsequencesRec(S, index+1, str);
  string str.push_back(S[index]);
  if (isPalindrome(str) && palindromes.find(str) == palindromes.end())
    c += 1;
  c += palindromicSubsequencesRec(S, index+1, str);
...
{
  return palindromicSubsequencesRec(S, 0);
